    ...
    def buildTree(self, preorder, inorder):
        def bulid(preorder,inorder):
            if len(inorder) == 1:               #出口
                ...
                root_val = preorder[0]
                fen = inorder.index(root_val)         ##查找值为root元素的下标（分割点）

                root = TreeNode(root_val)              #创建一个节点
                root.left = bulid(preorder[1:fen],inorder[:fen-1])             #左子树
                root.right = bulid(preorder[fen+1:],inorder[fen+1:])            #右子树
                return root
        return bulid(preorder,inorder)


class Solution1:                #答案1，，还有一种用栈的做法
